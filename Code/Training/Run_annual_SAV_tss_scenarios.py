...
SCENARIOS = {"baseline": 1.0, "TSS_plus25": 1.25, "TSS_minus25": 0.75, "TSS_minus50": 0.5}


def main() -> None:
    area_df, all_maps = run_scenarios("TSS_pred", SCENARIOS)
    area_df.to_csv(OUT_DIR / "tss_scenario_area_summary.csv", index=False)
    ...
    pd.concat(delta, ignore_index=True).to_csv(OUT_DIR / "tss_scenario_delta_vs_baseline.csv", index=False)
    ...
    fig.tight_layout()
    fig.savefig(OUT_DIR / "tss_scenario_timeseries.png", dpi=300)
    plt.close(fig)
