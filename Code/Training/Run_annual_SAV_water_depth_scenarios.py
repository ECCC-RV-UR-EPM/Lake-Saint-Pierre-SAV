...
from sav_annual_common import MAP_DIR, OUT_DIR, panel, run_custom_scenarios


def _shift_depth(delta_m: float):
    def transform(df):
        df["Water_depth"] = (df["Water_depth"] + delta_m).clip(lower=0.001)
        return df
...
SCENARIOS = {
    "baseline": _shift_depth(0.0),
    "WaterDepth_plus1m": _shift_depth(1.0),
    "WaterDepth_minus1m": _shift_depth(-1.0),
}


def main() -> None:
    area_df, all_maps = run_custom_scenarios(SCENARIOS)
    area_df.to_csv(OUT_DIR / "water_depth_scenario_area_summary.csv", index=False)
    ...
    pd.concat(delta, ignore_index=True).to_csv(OUT_DIR / "water_depth_scenario_delta_vs_baseline.csv", index=False)
    ...
    fig.tight_layout()
    fig.savefig(OUT_DIR / "water_depth_scenario_timeseries.png", dpi=300)
    plt.close(fig)
