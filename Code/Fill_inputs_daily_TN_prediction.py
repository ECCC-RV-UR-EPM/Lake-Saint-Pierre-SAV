...
prediction_dir = os.path.join(base_dir, "Prediction\TN_prediction_expanded")
...
files = sorted(
    (
        f for f in glob.glob(os.path.join(prediction_dir, "TN_prediction_*.txt"))
        if int(os.path.basename(f).split("_")[-1].split(".")[0]) >= START_YEAR
    ),
    key=lambda x: int(os.path.basename(x).split("_")[-1].split(".")[0])
)

print(f" Found {len(files)} TN yearly text files")
if len(files) == 0:
    raise FileNotFoundError(
        f"No TN yearly text inputs were found in {prediction_dir}."
    )
...
for file in files:
    ...
    print(f" Saved TN input parquet: {out_path}")

print("\n===================================")
print(" ALL TN INPUT FILES DONE")
print("===================================")
