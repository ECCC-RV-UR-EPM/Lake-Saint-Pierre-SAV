...
def _scale_temp(factor: float):
    def transform(df):
        df["Water_temp"] = df["Water_temp"] * factor
        return df
...
SCENARIOS = {
    "baseline": _scale_temp(1.0),
    "WaterTemp_plus10pct": _scale_temp(1.10),
    "WaterTemp_minus10pct": _scale_temp(0.90),
}


def main() -> None:
    area_df, all_maps = run_custom_scenarios(SCENARIOS)
    area_df.to_csv(OUT_DIR / "water_temp_scenario_area_summary.csv", index=False)
    ...
    pd.concat(delta, ignore_index=True).to_csv(OUT_DIR / "water_temp_scenario_delta_vs_baseline.csv", index=False)
    ...
    fig.tight_layout()
    fig.savefig(OUT_DIR / "water_temp_scenario_timeseries.png", dpi=300)
    plt.close(fig)
